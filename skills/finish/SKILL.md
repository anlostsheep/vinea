...
Propose only learning candidates that are stable, portable, verifiable, and not duplicates. Ask the user to accept or archive each candidate. Promote an accepted candidate only with `learning accept <task-id> --id <id> --confirmed-by user`; archive the rest with a stated reason. Never promote learning without that user decision.
