...
export async function createGoal(ctx: RepositoryContext, meta: Meta, input: { title: string; contract: ContractDraft; decision: Decision }): Promise<Task> {
  assertEntry(meta, null, "business-write"); contractDraftRule(input.contract); decisionRule(input.decision); text(input.title);
